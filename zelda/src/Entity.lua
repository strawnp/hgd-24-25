...
function Entity:render(adjacentOffsetX, adjacentOffsetY)
    ...
    local barX = self.x                        
    local barY = self.y + self.height + 2
    ...
    -- render health bar stuff
    if not self.isPlayer and then
        self.x, self.y = self.x - (adjacentOffsetX or 0), self.y - (adjacentOffsetY or 0)

        love.graphics.setColor(1, 0, 0, 1)
        love.graphics.rectangle('fill', barX, barY, barWidth, 3)

        love.graphics.setColor(0, 0.9, 0, 1)
        love.graphics.rectangle('fill', barX, barY, barWidth * healthPercentage, 3)
